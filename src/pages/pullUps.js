...
function PullUps() {
  ...
  const incrementPullUpCount = () => {
    pullUpCountRef.current += 1; // Updates immediately
    setPullUpCount(pullUpCountRef.current);
    console.log(`Immediate PullUps Count: ${pullUpCountRef.current}`);
  };
  ...
  const processPose = (keypoints) => {
    ...
    // planSets = parseInt(workoutPlan.sets);
    // planReps = parseInt(workoutPlan.reps);
    // planRestTime = parseInt(workoutPlan.restTime);
    // planSets = workoutPlan.sets;
    ...
    if (avgShoulderY < avgElbowY && !isPullingUpRef.current) {
      ...
      // console.log(`Came in 1.5 1.5 .15 .1.5 1.5 1.5 `);
      if (!isUpRef.current) {
        setIsUp(true);
        isUpRef.current = true
        // console.log("cam in avg shoulder 11111111111");
      } else {
        // setPullUpCount(pullUpCount + 1);
        // setPullUpCount(prevCount => prevCount + 1);
        incrementPullUpCount();
      console.log(`PullUps Count inside: ${pullUpCountRef.current}`);
  ...
  <div id="hide-when-next-clickked" className="buttons center">
    ...
    <a href="/">
      ...
      <div 
        ...
        style={{ display: (isVideoPlaying || videoEnded) && !videoEnded ? 'flex' : 'none' }}
      >
        <h2>Reps: <span id="repsCount">{pullUpCount}</span></h2>
