...
//   };
  ...
  const processPose = (keypoints) => {
    ...
    if(setCount >= planSetsRef.current-1 && dipCount >= planRepsRef.current){
      ...
      console.log(`Condition not true ${setCount} >= ${planSetsRef.current} and ${dipsCountRef.current} >= ${planRepsRef.current}`);
    }


    if(dipCount >= planRepsRef.current && !camStopped){
      checkRepsAndSets();
